(ns bis.main
  "Kotodama WASM entrypoint for the Bis clean-room actor (L4) — Clojure port.
  ...
  facts (`bis.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"bis.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bis-compat" :tier tier :entities entities} 200])
